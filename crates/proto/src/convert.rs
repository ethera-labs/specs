use ethera_spec::{ChainId, InstanceId, PeriodId, SuperblockNumber};

use crate::messages::{StartInstance, StartPeriod, TransactionRequest, XtRequest};

impl From<&ethera_spec::TransactionRequest> for TransactionRequest {
    fn from(r: &ethera_spec::TransactionRequest) -> Self {
        Self {
...
impl From<&TransactionRequest> for ethera_spec::TransactionRequest {
...
impl From<&ethera_spec::XtRequest> for XtRequest {
    fn from(r: &ethera_spec::XtRequest) -> Self {
        Self {
...
impl From<&XtRequest> for ethera_spec::XtRequest {
...
impl From<&ethera_spec::Instance> for StartInstance {
    fn from(i: &ethera_spec::Instance) -> Self {
        Self {
...
impl TryFrom<&StartInstance> for ethera_spec::Instance {
    ...
    fn try_from(si: &StartInstance) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            sequence_number: ethera_spec::SequenceNumber(si.sequence_number),
            xt_request,
        })
    }
}
...
#[cfg(test)]
mod tests {
    use ethera_spec::{ChainId, InstanceId, PeriodId};

    use crate::messages::*;

    #[test]
    fn xt_request_domain_roundtrip() {
        let domain = ethera_spec::XtRequest {
            transactions: vec![
        ...
        let back: ethera_spec::XtRequest = (&proto).into();
        assert_eq!(domain, back);
    }

    #[test]
    fn start_instance_domain_roundtrip() {
        let domain = ethera_spec::Instance {
            id: InstanceId([42; 32]),
            period_id: PeriodId(7),
            sequence_number: ethera_spec::SequenceNumber(3),
            xt_request: ethera_spec::XtRequest {
                transactions: vec![ethera_spec::TransactionRequest {
                    chain_id: ChainId(1),
        ...
        let back: ethera_spec::Instance = (&proto).try_into().unwrap();
        assert_eq!(domain, back);
    }
}
